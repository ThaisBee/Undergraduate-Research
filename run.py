    ...
    for s in slist:
        ...
        for u in electron_cloud_centers:
            Charge_strips = Strips_Integrator(
                strip_width, pitch, snoise, Position_strips
            ).Charge_Strip(s, u)
            (
                Charge,
                sCharge,
                ...
                snoise, N, u, Charge_strips, Position_strips, seed, threshold
            )
            Data.append(
                [
                    s,
...
def Plot_Charge_or_Error(
    Name, slist, df, markers, markersizes, ymin, ymax, Title, Ylabel
):
    for s in slist:
        t = r"$\sigma $ " + " " + str(s) + "mm"
        plt.errorbar(
            ...
            marker=markers[slist.index(s)],
            markersize=markersizes[slist.index(s)],
            linestyle="",
            label=t,
            xerr=0,
    ...
    for s in slist:
        Data.append(
            [
                ...
                RMS_uncertainty_propagation(
                    df.loc[df["s"].isin([s]), "E_linear"],
                    df.loc[df["s"].isin([s]), "sE_linear"],
                ),
                RMS(df.loc[df["s"].isin([s]), "E_quadratic"]),
                RMS_uncertainty_propagation(
                    df.loc[df["s"].isin([s]), "E_quadratic"],
                    df.loc[df["s"].isin([s]), "sE_quadratic"],
                ),
                RMS(df.loc[df["s"].isin([s]), "E_logarithmic"]),
                RMS_uncertainty_propagation(
                    df.loc[df["s"].isin([s]), "E_logarithmic"],
                    df.loc[df["s"].isin([s]), "sE_logarithmic"],
                ),
            ]
        )
...
def Plot_RMS(RMS_df, Labels):
    ...
    plt.errorbar(
        ...
        xerr=0,
        yerr=RMS_df["sE_quadratic"],
    )
    plt.errorbar(
        RMS_df["s"],
        RMS_df["E_logarithmic"],
        marker=markers[2],
        markersize=markersizes[2] * 1.5,
        linestyle="",
        label="Logarithmic weight",
        xerr=0,
        yerr=RMS_df["sE_logarithmic"],
    )
...
Plot_Charge_or_Error(
    "Q", slist, df, markers, markersizes, ymin, ymax, Title["Q"], Ylabel["Q"]
)
